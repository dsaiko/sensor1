...
int64_t lastBlankAtMs = 0;
int64_t blankUntilMs = 0;
...
void maybeBlank()
{
    ...
    if (blankUntilMs != 0)
    {
        ...
        // Reset blank timer to avoid immediate re-blanking
        lastBlankAtMs = nowMs;
        return;
    }

    if (nowMs - lastBlankAtMs >= kBlankPeriodMs)
    {
        lastBlankAtMs = nowMs;
        blankUntilMs = nowMs + kBlankDurationMs;
        u8g2.setPowerSave(1);
    }
}
} // namespace
